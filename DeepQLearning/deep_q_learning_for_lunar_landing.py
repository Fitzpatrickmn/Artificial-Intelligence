...
def show_video_of_model(agent, env_name, epsilon=0.01):
    ...
    while not done:
        frame = env.render()
        frames.append(frame)
        action = agent.act(state, epsilon)  # Consider exploration
        state, reward, done, _, _ = env.step(action.item())
    env.close()
    imageio.mimsave('DeepQLearning/LunarLanderVideo03132024.mp4', frames, fps=30)
